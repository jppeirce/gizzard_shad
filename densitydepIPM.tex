...
We use both Long Term Resource Modeling fish data and parameters fit from other empirical studies to  introduce an integral projection model (IPM) for gizzard shad. 
IPMs are a generalization of stage-based, matrix population models that have been used to describe a wide range of organisms. 
They are a natural choice for gizzard shad since many aspects of their life cycle have been studied. 
We test model outputs against empirical patterns reported for this fish species at a key location along the Illinois River (La Grange Reach). 
...
Logistic survival probability function, $s(z)$ & & \\
  ...
  & THEN 2015 \\ 
 $\ds \alpha_s$  & inflection point & 79.53 & Estimated from LTRM dataset \\
  $\ds \beta_s$ & slope & -156.71 & Estimated from LTRM dataset \\\hline
...
In this paper, the life cycle of gizzard shad is partitioned into two compartments: 1) survival and growth, and  2) reproduction. 
...
We define $P(z', z) = s(z)G(z',z)$ where $s(z)$ is the annual adult survival probability and the growth $G(z',z)$ describes the annual length transitions. 
...
Finally, the total number of eggs that result in an age-1 fish is multiplied with a normal distribution of length,
...
The goal of the LTRM is track changes in the condition and/or stability of the UMRS ecosystem and participate in restoration activities throughout the region. 
...
Fish are captured using a multiple gear approach (including netting and electrofishing) in order to monitor the responses and health of fish communities along these two very important waterways over time \citep{gutreuter1995long}. 
...
We employed this approach as the La Grange Reach is upstream of the Mississippi River making it a more distinct location compared to the other sites. 
...
The parameters for the size distribution of age-1 fish were gleaned from a study of gizzard shad located in large impoundments \citep{michaletz2017variation} and the historic 1990-2020 LTRM dataset from the main channel of the UMRS (outlined above).
...
The density of age-0 gizzard shad strongly influenced the density of these fish at subsequent developmental stages.
...
If this reduced survival continues for a few years, the overall density of fish within the population may decline and result in a fewer number of longer length fish that are reproducing.  
...
\begin{subfigure}[b]{.63\textwidth}
   \includegraphics[width=.45\textwidth]{figures/period_small.png}
      \includegraphics[width=.45\textwidth]{figures/period_large.png}
     \caption{}
\label{fig:period}
\end{subfigure}
\caption{(a) The total number of gizzard shad in La Grange Reach predicted by IPM for first 100 years. 
(b) Simulated length distributions during an 9 year interval of time (approximately 1 period of the total density function)}
%(b) Simulated length distributions during an 9 year interval of time (approximately 1 period of the total density function)}
\end{figure}    
...
After simulating an additional 50 years, we fit a periodic function to the annual density of gizzard shad and determined a period of approximately 8.74 years. 
...
The ebb and flow of the frequency of the age-1 cohort are associated with the density-dependent survival function for age-0 fish (explained in Section \ref{sec:survival}).

Figure \ref{fig:period} captures the cohort length dynamics of the gizzard shad. 
The peak length frequency of age-1 fish in Year 1 near $z = 105$ mm is seen as a peak in Year 3 near 185 mm, a peak near 305 mm in Year 3, before becoming nearly unidentifiable in the graphs for Years 7 and 9.  
This is consistent with the lifespan reported for gizzard that can range, on average, between 4 and 6 years \citep{bodola1955life}. 
...
%[Years with low density of fish less than 200 mm are followed by years where the density is profile is more bell shaped or favoring high density.]
LETS TALK ABOUT THIS FRIDAY]
