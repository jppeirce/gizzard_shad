...
Five of the study sites (Pools 4, 8, 13, 26, and the Open River Reach) correspond to field stations located along the main channel of the river (Figure \ref{fig:field_stations}).
...
Invasive carp are first recorded in the LTRM in XXX year.  (JP IS CHECKING IF WORTH DOING?)
Since carp have been shown to affect gizzard shad populations, we used only the data collected before 2000.
...
First, we parameterized the IPM using data from the main channel of the UMR.  
As a part of Illinois River and UMR, La Grange Reach is upstream from the main channel (should we include figure/map?).  
The proximity to, but relative independence from, the main channel, made it a good choice.  
Secondly, La Grange Reach is a large pool between the main channel of the Mississippi River and Lake Michigan.  
...
The parameters for the size distribution of age-1 fish were gleaned from a study of gizzard shad located in large impoundments \citep{michaletz2017variation} and the historic 1990-2020 LTRM dataset of the main channel of the UMRS.
...
Studies [CITATION] suggest that due to the environmental stochasticity and other effects, smaller recruitment fish densities can fluctuate annually and be difficult to measure accurately. MOVED THIS SECTION TO DISCUSSION% 
...
Gizzard shad are common in freshwater systems of North America where they are important components fo the freshwater community. Given this, it is important to understand how populations of this species vary with changing environmental circumstances, such as the occurrence of invasive species. Herein we use both LTRM data and parameters gleaned from other empirical studies to develop an integral projection model for gizzard shad and test model outputs against patterns reported from a well-studied population of this species from the Illinois River.   
...
In our study, the density-dependent survival of the age-0 cohort resulted periodic fluctuations in the density of gizzard shad in La Grange reach. 
For other parameter sets (corresponding to other ecosystems), lower survival probability of adults may not lead to the same asymptotic behavior.  
In fact, we suspect that reduced survival rates may produce a model density that tends towards stable equilibrium as time increases.  
Further investigation of possible bifurcation values in the parameter set could be a direction for future study.
