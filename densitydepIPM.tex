...
Logistic survival probability function, $s(z)$ & & \\
  ...
  & THEN 2015 \\ 
 $\ds \alpha_s$  & inflection point & 103.53 & Estimated from LTRM dataset \\
  $\ds \beta_s$ & slope & -943.89 & Estimated from LTRM dataset \\\hline
...
The length distributions during the periodic orbit (Figure \ref{fig:period}) compares favorably with the length observations from La Grange found in the LTRMP fish dataset (Figure \ref{fig:LTRMlg}). 
\begin{figure}
\centering
\begin{subfigure}[b]{.43\textwidth}
...
\label{fig:period}
\end{subfigure}
\caption{(a) The total number of gizzard shad in La Grange Reach predicted by IPM for first 150 years. (b) USE-later - LTRMP observations of gizzard shad in La Grange Reach (histogram with 50 len) compared with the IPM length frequency at equilibrium ($t=500$ years)}
\end{figure}    
The average simulated length distribution has similarities to the average observations from La Grange found in the LTRMP fish dataset (Figure \ref{fig:lagrange}). 
\begin{figure}
\centering
\begin{subfigure}[b]{.43\textwidth}
