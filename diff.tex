...
%DIF DEL densitydepIPM_pre-usgs_review.tex   Tue Jun 28 10:34:59 2022
%DIF ADD densitydepIPM.tex                   Fri Jul  1 13:21:57 2022
...
\begin{figure}
    \begin{center}
%\scalebox{.75}[.75]{
\DIFdelbeginFL %DIFDELCMD < \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
%DIFDELCMD <   thick,
%DIFDELCMD <   main node/.style={rectangle,draw},
%DIFDELCMD <   box/.style = {draw=gray, very thick,
%DIFDELCMD <                             minimum height=11mm, text width=11mm, 
%DIFDELCMD <                             align=center},]
%DIFDELCMD <                               

%DIFDELCMD < \node[main node, align=center] (census_pres) at (0,3) {Census $t$ \\
%DIFDELCMD <         \includegraphics[width=0.12\textwidth]{adult.jpeg} \\
%DIFDELCMD <         $n(z,t)$};
...
%DIFDELCMD <         \includegraphics[width=0.15\textwidth]{adult.jpeg}\\
%DIFDELCMD <         $n(z',t+1)$};
%DIFDELCMD < \node (repro_box) at (13,3.85)  {Reproduction};
%DIFDELCMD < \node[align=center] (eggs) at (3,0) {Eggs \\ $\bullet$ $\bullet$ $\bullet$ $\bullet$ \\
%DIFDELCMD <   \, $\bullet$ $\bullet$ $\bullet$ $\bullet$ \\ $\bullet$ $\bullet$ $\bullet$ $\bullet$};
%DIFDELCMD <   \node[align=center] (eggs_viable) at (5,0) {Viable \\ $\bullet$ $\bullet$ $\bullet$ \\
%DIFDELCMD <   \, $\bullet$ $\bullet$ \\ $\bullet$ $\bullet$ $\bullet$};
%DIFDELCMD <  \node[align=center] (age1) at (8,0) { Age-1\\
%DIFDELCMD <         \includegraphics[width=0.06\textwidth]{adult.jpeg}
%DIFDELCMD <          \includegraphics[width=0.04\textwidth]{adult.jpeg}\\
%DIFDELCMD <          \includegraphics[width=0.08\textwidth]{adult.jpeg}
%DIFDELCMD <         };
%DIFDELCMD < \node[] (next) at (13,3) {};
%DIFDELCMD < 

%DIFDELCMD < \path[->]
%DIFDELCMD < 	(census_pres) edge node {} (repro_in)
%DIFDELCMD < 	(repro_out) edge node {$s(z)G(z',z)$} (census_next)
%DIFDELCMD < 	(census_next) edge node {} (next)
%DIFDELCMD < 	(repro_down) edge node {$p_b \rm{egg}(z)$} (eggs)
%DIFDELCMD < 	(eggs) edge node {$\nu$} (eggs_viable)
%DIFDELCMD < 	(eggs_viable) edge node {$s_0(d)$} (age1)
%DIFDELCMD < 	(age1) edge node[right, pos=0.3] {$C_1(z')$} (census_next);
%DIFDELCMD < 	

%DIFDELCMD < \end{tikzpicture}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
  thick,
...
\path[->]
	...
	(repro_down) edge node {$p \rm{egg}(z)$} (eggs)
	(eggs) edge node {$\nu$} (eggs_viable)
...
%DIF >  to make up for having a graphical abstract
...
%DIFDELCMD < \begin{tabular}{p{6.5cm}|p{6cm}|c| p{6cm}  }\hline
%DIFDELCMD <      Parameter & Meaning (units) & Mean& Source \\\hline
%DIFDELCMD < Logistic survival probability function, $s(z)$ & & \\
%DIFDELCMD <   $\ds s_{\rm {min}}$ & minimum survival & 0.002 & \citep{bodola1955life} \\
%DIFDELCMD <   $\ds s_{\rm {max}}$ & maximum survival & $\ds 1-8.871K^{0.73}L_{\infty}^{-0.33}$ 
%DIFDELCMD <   & \citep{then2015evaluating} \\ 
%DIFDELCMD <  $\ds \alpha_s$  & inflection point & 80.01 & Estimated from LTRM dataset \\
%DIFDELCMD <   $\ds \beta_s$ & slope & -139.93 & Estimated from LTRM dataset \\\hline
%DIFDELCMD <   Growth function, $\ds G(z,z')$ & & \\
%DIFDELCMD <   $L_\infty$ & maximum length (in mm) & 394.30 & \citep{catalano2010size} \\
%DIFDELCMD <   $K_g$ & individual growth rate & 0.26 &  \citep{michaletz2017variation} \\
%DIFDELCMD <   $\sigma_g$ & growth standard deviation & 25 &  \citep{michaletz2017variation} \\\hline
%DIFDELCMD <   Normal distribution of length of age-1, $\ds C_1(z')$ & & \\
%DIFDELCMD <   $\mu_r$ & mean length of recruitment (in mm) & 105 & \citep{michaletz2017variation} \\
%DIFDELCMD <   $\sigma_r$  & standard deviation of length & 25 & \citep{michaletz2017variation} \\\hline
%DIFDELCMD < Eggs produced, egg(z) & & \\
%DIFDELCMD < $\ds \mbox{egg}_{\mbox{max}}$ & maximum number of eggs produced & 737512.1 & Estimated from \citep{jons1997ovarian} \\
%DIFDELCMD < $\ds \alpha_e$ & inflection point & 314.44 & Estimated from \citep{jons1997ovarian} \\
%DIFDELCMD < $\ds \beta_e$ & slope & -7.18 & Estimated from \citep{jons1997ovarian} \\\hline
%DIFDELCMD < Survival of age-0, $s_0(d(t))$ & & \\
%DIFDELCMD < $\ds a_0$ & intercept & 0.27 & Estimated from \citep{michaletz2010overwinter} \\
%DIFDELCMD < $\ds b_0$ & decay rate & 0.003 & Estimated from \citep{michaletz2010overwinter} \\\hline
%DIFDELCMD < Spawning & & \\ 
%DIFDELCMD <  $\nu$  & probability that egg becomes viable & 0.002 & \citep{bodola1955life} \\
%DIFDELCMD <  $p_b$ & probability that female spawns & 0.90 &    \\ 
%DIFDELCMD <  \end{tabular} } \label{table:parameters}
%DIFDELCMD <      %%%
...
Spawning & & \\ 
 $\nu$  & probability that egg becomes viable & 0.002 & \citep{bodola1955life} \\
 $p$ & probability that female spawns & 0.90 &    \\ 
...
Mature gizzard shad tend to mate between May and June, although this can vary based on water temperatures \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{bodola1955life}}\hskip0pt%DIFAUXCMD
}\DIFaddend . 
...
In many habitats, individuals can reach sexual maturity within a year \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{bodola1955life}}\hskip0pt%DIFAUXCMD
}\DIFaddend . 
...
Given the large number of eggs produced by shad females (\DIFaddbegin \DIFadd{often }\DIFaddend $>$ \DIFdelbegin \DIFdel{300}\DIFdelend \DIFaddbegin \DIFadd{100}\DIFaddend ,000/year) \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{jons1997ovarian}}\hskip0pt%DIFAUXCMD
}\DIFaddend , there is evidence that intraspecific competition can be intense during early developmental stages in this species leading to density-dependent mortality \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{michaletz2010overwinter}}\hskip0pt%DIFAUXCMD
}\DIFaddend . 
...
We used an IPM to describe the life history of \DIFaddbegin \DIFadd{female }\DIFaddend gizzard shad in the Upper Mississippi River system (UMRS). 
...
Since that time, IPMs have been used to describe a wide range of organisms \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{ellner2016data, merow2014advancing, rees2014building}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{briggs2010structured, ellner2016data, merow2014advancing, rees2014building}}\hskip0pt%DIFAUXCMD
}\DIFaddend , but have only recently \DIFdelbegin \DIFdel{be }\DIFdelend \DIFaddbegin \DIFadd{been }\DIFaddend used to model fish populations, such as \citep{erickson2017integral, liao2019dynamic, white2016fitting, pollesch2022developing}. 
...
In our model specifically, we incorporate data from studies on 1) egg production in different size classes of adults, 2) density-dependent survival of age-0 shad, and 3) length distributions of gizzard shad in the UMRS that span nearly thirty years \DIFaddbegin \DIFadd{(Table \ref{table:parameters})}\DIFaddend .

\DIFdelbegin \DIFdel{We assumed that variations among individual }\DIFdelend \DIFaddbegin \DIFadd{In a traditional matrix population model \mbox{%DIFAUXCMD
\citep{caswell2000matrix}}\hskip0pt%DIFAUXCMD
, the annual abundance of }\DIFaddend gizzard shad can be \DIFdelbegin \DIFdel{summarized by its length$z$ (in mm) ranging from the minimum possible length $L$ to the maximum value $U$.
The }\DIFdelend \DIFaddbegin \DIFadd{separated by factors such as age, length, developmental stage, etc. into a finite number of disjoint subgroups.
Within each subgroup, individual fish can be assumed to exhibit the same survival, growth, and fecundity.
These parameters are used to build a transition matrix that updates the current }\DIFaddend state of the population at time $t$ \DIFdelbegin \DIFdel{(in years) }\DIFdelend \DIFaddbegin \DIFadd{to the future state at $t+1$ through matrix multiplication.
Increasing the number of subgroups also increases the number of parameters that require estimation when generating the transition matrix.
Conversely, the annual update in gizzard shad abundance for an IPM }\DIFaddend is described by the \DIFdelbegin \DIFdel{length distribution $n(z,t)$.
Specifically, for each time $t$, }\DIFdelend \DIFaddbegin \DIFadd{integral kernel.
This kernel can be created from continuous functions for survival, growth, and fecundity and therefore IPMS almost always require fewer estimated parameters \mbox{%DIFAUXCMD
\citep{briggs2010structure}}\hskip0pt%DIFAUXCMD
.
When the integral kernel is applied to the current length distribution }\DIFaddend $n(z,t)$ \DIFdelbegin \DIFdel{is a smooth function of $z$ such that the }\DIFdelend \DIFaddbegin \DIFadd{and integrated over the length $[L,U]$, the result is the next length distribution $n(z',t+1)$.
$n(z',t+1)$ describes the }\DIFaddend number of individuals of length \DIFdelbegin \DIFdel{$z$ }\DIFdelend \DIFaddbegin \DIFadd{$z'$ }\DIFaddend in the interval \DIFdelbegin \DIFdel{$[a,b]$ at time $t$ is $\ds \int_a^b n(z,t) \, dz$. 
Between times $t$ and }\DIFdelend \DIFaddbegin \DIFadd{$[a', b']$ at time }\DIFaddend $t+1$\DIFdelbegin \DIFdel{, individual gizzard shad may grow, die, and produce offspring that vary in length depending on the individual's current length (Figure \ref{life_cycle}).
}\DIFdelend \DIFaddbegin \DIFadd{.
For the numerical simulations in Section \ref{sec:analysis}, the length vector will be discretized.
As a result, $n(z,t)$ becomes a vector and the integral kernel acts on $n(z,t)$ like a large approximating transition matrix.
In the following sections, we detail the functions used to create the integral kernel for the gizzard shad IPM
}\DIFaddend 
...
F(z', z) = p \DIFdelbegin \DIFdel{_b }\DIFdelend \, \mbox{egg}(z) \, \nu \, s_0(d(t)) \, C_1(z')
\end{equation}
where \DIFdelbegin \DIFdel{$p_b$ }\DIFdelend \DIFaddbegin \DIFadd{$p$ }\DIFaddend is the probability of reproducing, $\mbox{egg}(z)$ is the mean number of eggs produced \DIFaddbegin \DIFadd{by a fish of length $z$}\DIFaddend , $\nu$ is the probability that an egg is viable, $s_0(d(t))$ is the density-dependent probability of surviving to age-1, and $C_1 (z')$ is the length distribution of new recruits at age-1 (when they are first censused in the model).
...
\[ d(t) = 10^{-3} \int_L^U p \DIFdelbegin \DIFdel{_b }\DIFdelend \, \mbox{egg}(z) \, \nu n(z,t) \, dz. \]  

Finally, \DIFaddbegin \DIFadd{Equation \ref{eq:fecundity} is constructed from mulitplying }\DIFaddend the total number of eggs that survive to be an age-1 fish \DIFdelbegin \DIFdel{is multiplied }\DIFdelend with a normal distribution of length,
...
%DIF >  is the length distribution of new recruits added to the population at time $t+1$.
...
\end{equation}  
where \DIFaddbegin \DIFadd{the integral kernel }\DIFaddend $K(z',z) = s(z) G(z',z) + F(z',z)$ and $[L,U]$ is the range of all possible lengths.

\section{Methods}
\subsection{\DIFaddbegin \DIFadd{Long Term Resource Monitoring (}\DIFaddend LTRM\DIFaddbegin \DIFadd{) }\DIFaddend data and model parameterization} \label{LTRM}
...
To parameterize our model, we used gizzard shad data collected from the 5 sites \DIFaddbegin \DIFadd{(Pools 4, 8, 13, 26, and Open River Reach) }\DIFaddend along the Mississippi River \DIFaddbegin \DIFadd{(Figure \ref{fig:field_stations})}\DIFaddend .  
...
%DIFDELCMD <   \includegraphics[width=.6\textwidth]{figures/field_stations.png}
%DIFDELCMD <   %%%
...
\end{subfigure}
\DIFdelbeginFL %DIFDELCMD < \begin{subfigure}[b]{.43\textwidth} 
%DIFDELCMD <    \includegraphics[width=1.2\textwidth]{figures/LTRMmain.png}
%DIFDELCMD <    %%%
...
\subsection{Initial conditions}  We assumed that the initial density of gizzard shad was $d_0 = 964.7$, the \DIFdelbegin \DIFdel{annual average }\DIFdelend \DIFaddbegin \DIFadd{average annual }\DIFaddend density of gizzard shad \DIFdelbegin \DIFdel{observed in }\DIFdelend \DIFaddbegin \DIFadd{caught with all gear types represented in the LTRM fish program in }\DIFaddend La Grange Reach from 1993-2020.  
...
In the simulated solutions to the IPM, the density of age-0 \DIFdelbegin \DIFdel{Fish strongly influenced }\DIFdelend \DIFaddbegin \DIFadd{fish are strongly influenced by }\DIFaddend the density of gizzard shad at subsequent developmental stages.
...
\begin{figure}
\centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.4\textwidth]{figures/Figure2a.pdf}
%DIFDELCMD <    %%%
...
\begin{figure}
\centering
\DIFdelbeginFL %DIFDELCMD < \begin{subfigure}[b]{.45\textwidth}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigure}[b]{.48\textwidth}
...
\centering
  \DIFdelbeginFL %DIFDELCMD < \begin{subfigure}[b]{.43\textwidth}
%DIFDELCMD <   \includegraphics[width=\textwidth]{figures/LTRMlg.png}
%DIFDELCMD <    %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
}
...
%DIFDELCMD < \begin{subfigure}[b]{.43\textwidth}
%DIFDELCMD <    \includegraphics[width=\textwidth]{figures/lagrange.pdf}
%DIFDELCMD <      %%%
\DIFdelendFL \caption{\DIFdelbeginFL %DIFDELCMD < \MBLOCKRIGHTBRACE
%DIFDELCMD < \label{fig:lagrange}
...
In addition, studies also suggest that environmental stochasticity and food variability may alter recruitment densities which are difficult to measure accurately \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{rose2000quantitative, okamoto2016stochastic}}\hskip0pt%DIFAUXCMD
. 
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{michaletz2010overwinter, okamoto2016stochastic, rose2000quantitative}}\hskip0pt%DIFAUXCMD
. 
Future investigations may use spring water temperature measurements from the LTRM Water Quality component to modify the adult survival function (Equation \ref{eq:surv}) to be temperature dependent.
}\DIFaddend 
...
The location of the maximum length and the variation in the \DIFdelbegin \DIFdel{of }\DIFdelend length of new recruits recorded in the LTRM data suggests that there may be smaller age-0 fish in La Grange Reach than in the study location \citep{michaletz2017variation} used to parameterize the model.  
